...
export const WorkspacePanel = forwardRef<WorkspacePanelRef>((_props, ref) => {
  ...
  }, [shortDescription, localPrompt, promptMode]);

  // Live-update edit prompt when in edit mode
  useEffect(() => {
    if (promptMode !== 'edit') return;
    
    const unsubscribe = editTracker.subscribe(() => {
      const newPrompt = editTracker.getModificationPrompt();
      setAutoGeneratedPrompt(newPrompt);
  ...
  const handleModeChange = (mode: PromptMode): void => {
    ...
    if (mode === 'edit') {
      ...
      // Switch back to generate mode - restore the short description
      setLocalPrompt(shortDescription);
      setAutoGeneratedPrompt('');
    }
  };
